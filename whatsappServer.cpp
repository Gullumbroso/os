...
void whatsappServer::connection(){
    ...
    fd_set fds;
    int returnVal;
    initConnection(newConnection, fds, returnVal);
    if (returnVal == ERROR){
    ...
    printSituation((string)message," failed to connect.");
    close(newConnection);
...
void whatsappServer::initConnection(int &newConnection, fd_set &fds, int &returnVal) const {
    ...
    timer.tv_sec = 2;
    timer.tv_usec = 0;
    returnVal= select(FD_SETSIZE, &fds, NULL, NULL, &timer);
...
void whatsappServer::createGroup(string &user, int fd, const string &msg, size_t space2, const string &nameOfGroup) {
    vector<string> clientsNames;
    string clients = msg.substr(space2 + 1);
    size_t curComma = clients.find(",");
    //adding the clients to the vector
    while (curComma != basic_string::npos) {
